...
// from http://www.opengaussql.org/docs/current/static/sql-keywords-appendix.html
var (
	...
	// DefaultopengaussSchema default opengauss schema
	DefaultopengaussSchema = "public"
)
...
const opengaussPublicSchema = "public"

type opengauss struct {
	core.Base
}

func (db *opengauss) Init(d *core.DB, uri *core.Uri, drivername, dataSourceName string) error {
	err := db.Base.Init(d, db, uri, drivername, dataSourceName)
	if err != nil {
		return err
	}
	if db.Schema == "" {
		db.Schema = DefaultopengaussSchema
	}
	return nil
}

func (db *opengauss) SqlType(c *core.Column) string {
	var res string
...
func (db *opengauss) SupportInsertMany() bool {
	return true
}

func (db *opengauss) IsReserved(name string) bool {
	_, ok := opengaussReservedWords[name]
	return ok
}

func (db *opengauss) Quote(name string) string {
	name = strings.Replace(name, ".", `"."`, -1)
	return "\"" + name + "\""
}

func (db *opengauss) AutoIncrStr() string {
	return ""
}

func (db *opengauss) SupportEngine() bool {
	return false
}

func (db *opengauss) SupportCharset() bool {
	return false
}

func (db *opengauss) IndexOnTable() bool {
	return false
}

func (db *opengauss) IndexCheckSql(tableName, idxName string) (string, []interface{}) {
	if len(db.Schema) == 0 {
...
func (db *opengauss) TableCheckSql(tableName string) (string, []interface{}) {
	if len(db.Schema) == 0 {
...
func (db *opengauss) ModifyColumnSql(tableName string, col *core.Column) string {
	if len(db.Schema) == 0 {
...
func (db *opengauss) DropIndexSql(tableName string, index *core.Index) string {
	quote := db.Quote
	idxName := index.Name
...
func (db *opengauss) IsColumnExist(tableName, colName string) (bool, error) {
...
func (db *opengauss) GetColumns(tableName string) ([]string, map[string]*core.Column, error) {
	args := []interface{}{tableName}
...
func (db *opengauss) GetTables() ([]*core.Table, error) {
	args := []interface{}{}
...
func (db *opengauss) GetIndexes(tableName string) (map[string]*core.Index, error) {
	args := []interface{}{tableName}
...
func (db *opengauss) Filters() []core.Filter {
	return []core.Filter{&core.IdFilter{}, &core.QuoteFilter{}, &core.SeqFilter{Prefix: "$", Start: 1}}
}

func parseGaussURL(connstr string) (string, error) {
	u, err := url.Parse(connstr)
	if err != nil {
		return "", err
	}

	if u.Scheme != "opengauss" {
...
func (p *gaussDriver) Parse(driverName, dataSourceName string) (*core.Uri, error) {
	db := &core.Uri{DbType: OPENGAUSS}
	var err error

	if strings.HasPrefix(dataSourceName, "opengauss://") {
		db.DbName, err = parseGaussURL(dataSourceName)
		if err != nil {
			return nil, err
		}
	} else {
...
func init() {
	core.RegisterDriver(OPENGAUSS, &gaussDriver{})
	core.RegisterDialect(OPENGAUSS, func() core.Dialect {
		return &opengauss{}
	})
}
